...
`/feedback The agent stopped before finishing the tests`. T3 Code uploads the conversation and
Codex logs to OpenAI. You can share the returned thread ID with OpenAI support.

## Sub-agent models

The web, desktop, and mobile Subagents view shows each sub-agent's model and reasoning effort when Codex
