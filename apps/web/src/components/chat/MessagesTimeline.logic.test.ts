...
describe("deriveMessagesTimelineRows", () => {
  ...
  it.each([
    ...
    (_, statuses, hasFailure) => {
      ...
      expect(rows.find((row) => row.kind === "work-toggle")).toMatchObject({
        hiddenCount: 3,
        summary: "Used 2 tools and received 1 update",
        hasFailure,
      });
    },
  );

  it("keeps error-toned entries separate from mixed work groups", () => {
    const timelineEntries = ["error", "info", "completed"].map((status, index) => {
      const id = `work-${index}`;
      const createdAt = `2026-01-01T00:00:0${index}Z`;
      return {
        id: `work-entry-${index}`,
