...
export function SettingsUsageRouteScreen() {
  ...
  const environmentMenu =
    environments.length > 1 ? (
      ...
      >
        <Pressable
          accessibilityLabel="Choose usage environment"
  ...
  return (
    <View collapsable={false} className="flex-1 bg-sheet">
      ...
      >
        <SettingsSection title="Subscription usage" card>
          <View className="gap-4 p-4">
            <View className="gap-2 rounded-2xl bg-subtle px-4 py-3">
              <Text className="text-sm leading-relaxed text-foreground-muted">
                ...
                that do not publish a stable usage interface.
              </Text>
            ...
            {isLoadingSavedConnection ? (
              ...
              <View className="min-h-40 items-center justify-center gap-3 px-4 py-6">
                ...
                <Text className="text-center text-sm leading-relaxed text-foreground-muted">
                  Add a server in Settings → Environments to load subscription usage.
                </Text>
              </View>
              ...
              <View className="min-h-40 items-center justify-center gap-3 rounded-2xl bg-subtle px-5 py-6">
                ...
                />
                <Text className="text-center text-base font-t3-medium text-foreground">
                  Usage reporting isn’t available
                </Text>
                <Text className="text-center text-sm leading-relaxed text-foreground-muted">
                  This T3 server doesn’t support subscription usage reporting yet. Update the T3
              ...
              <View className="min-h-48 items-center justify-center gap-3">
                <ActivityIndicator color={iconColor} />
                <Text className="text-sm text-foreground-muted">Loading subscription usage…</Text>
              </View>
            ) : usage.data ? (
              <View className="gap-4" onLayout={handleGridLayout}>
                {usage.error ? (
                  ...
                  >
                    ...
                    >
                      {errorKind === "unsupported-server"
                        ? "This server no longer supports usage reporting. Update it to refresh the snapshot below."
                        : "Usage could not be updated. The last successful snapshot is still shown."}
                    </Text>
                  </View>
                ) : null}
                ...
                {usage.data.cards.length > 0 ? (
                  ...
                  <View className="min-h-32 items-center justify-center rounded-2xl border border-dashed border-border px-5 py-6">
                    <Text className="text-center text-sm text-foreground-muted">
                      No subscription usage is available for this server.
                    </Text>
                  </View>
                )}
              </View>
              ...
              <View className="min-h-32 items-center justify-center rounded-2xl border border-dashed border-border px-5 py-6">
                <Text className="text-center text-sm text-foreground-muted">
                  Subscription usage is not available yet.
                </Text>
              </View>
            )}
          </View>
