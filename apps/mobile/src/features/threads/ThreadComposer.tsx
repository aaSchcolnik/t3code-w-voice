...
export const ThreadComposer = memo(function ThreadComposer(props: ThreadComposerProps) {
  ...
  const handleVoiceCommit = useCallback(
    (transcribedText: string) => {
      const selection = composerMenu.selection;
      const prefix =
        selection.start > 0 && !/\s$/u.test(props.draftMessage.slice(0, selection.start))
          ? " "
          : "";
      const suffix =
        selection.end < props.draftMessage.length &&
        !/^\s/u.test(props.draftMessage.slice(selection.end))
          ? " "
          : "";
    ...
    [composerMenu, inputRef, props.draftMessage, props.onChangeDraftMessage],
  );
