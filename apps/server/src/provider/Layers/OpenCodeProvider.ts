...
> {
  ...
  const inventoryExit = yield* Effect.exit(
    inventoryEffect.pipe(
      ...
      // Interrupt either the external-server inventory request or CLI probe so
      // provider discovery cannot remain pending forever.
  ...
  const inventoryResult = inventoryExit.value.value;
